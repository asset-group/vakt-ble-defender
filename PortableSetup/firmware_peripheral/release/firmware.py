...
pio_build_path = Path('.pio/build/peripheral_nrf52840/')
...
def flash_firmware(serial_port, application_only=False):
    ...
    os.system(f'{sys.executable} runtime/python/install/bin/adafruit-nrfutil dfu genpkg --dev-type 82 --application-version 1 \
        --application %s \
        --sd-req 0xB6 \
        binaries/app_dfu_package.zip' % (firmware_path))
    ...
    if len(args):
        ...
        for i, arg in enumerate(args):
            if 'build' in arg and enable_build:
                ...
                with ZipFile(Path('release/vaktble_firmware_peripheral.zip'), 'w', ZIP_DEFLATED) as zipObj:
