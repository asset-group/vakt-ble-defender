...
{
  // Disable TX FEM
  nrf_gpio_pin_write(FEM_TXEN, 0);
  // Enable FEM
  nrf_gpio_pin_write(FEM_RXEN, 1);
  ...
  // NRF_RADIO->INTENSET = RADIO_INTENSET_PAYLOAD_Msk | RADIO_INTENSET_END_Msk;
  // NRF_RADIO->INTENSET = RADIO_INTENSET_PAYLOAD_Msk | RADIO_INTENSET_END_Msk | RADIO_INTENCLR_BCMATCH_Msk;
  NRF_RADIO->INTENSET = RADIO_INTENSET_BCMATCH_Msk | RADIO_INTENSET_END_Msk;
  NRF_RADIO->SHORTS = RADIO_SHORTS_READY_START_Msk | RADIO_SHORTS_END_START_Msk | RADIO_SHORTS_ADDRESS_BCSTART_Msk; // Start bit counter on every pkt
  // Interrupt when bits correspoindng to Advertising Address (peripheral) to arrive
  NRF_RADIO->BCC = 112; // This corresponds to advertising address bit offset
  // NRF_RADIO->BCC = 112 + 88 + 16 + 16 + 16 + (4 * 8) + 5;
  // NRF_RADIO->BCC = 112 + 88 + 16 + 16 + 16;
  // NRF_RADIO->BCC = 104 - (1 * 8);

  // Enable NVIC Interrupt for Radio
  NVIC_SetPriority(RADIO_IRQn, IRQ_PRIORITY_HIGHEST);
...
void radio_send_custom(uint8_t *pBuffer, uint8_t channel, uint32_t access_address, uint32_t crc_init)
{
  ...
  // Question: Does that mean that Radio_IRQHandler is triggered after every radio_send_custom?
}
...
void radio_tx_to_rx(uint8_t en_bcc)
{
  // Disable TX FEM
  nrf_gpio_pin_write(FEM_TXEN, 0);
  // Enable RX FEM
  ...
  if (en_bcc)
  {
    NRF_RADIO->BCC = 112; // (8 + 13 Bytes * 8 Bits)
