...
// Common macros
#define WD_HANDLER_PKT_DRV_TYPE function<void(wd_t *, WDEventQueue<V> &, T &)>
#define WD_HANDLER_PKT_TYPE function<void(wd_t *, WDEventQueue<V> &)>
#define WD_HANDLER_PKT_SMPL_DRV_TYPE function<void(WDEventQueue<V> &, T &)>
...
typedef struct {
    bool label_status;
    std::string pkt_label;
    std::string pkt_summary;
...
typedef struct _pkt_evt_t {
    ...
    uint8_t o_pkt_buf[256]; // Original Packet Buffer
    uint8_t _pkt_buf[256];  // Current Packet Buffer
    ...
    inline void save_packet(uint8_t *pkt, uint32_t length, uint16_t offset = 0)
    {
        ...
        if (unlikely(pkt_real_len > sizeof(_pkt_buf)))
            return;
    ...
    static void signal_handler(int signal)
    {
        ...
        if (!cls->Exiting &&
            ...
            cls->watchdog_loop.onTimeout(2, []() {
                GL1R("Exit Timeout (2s)!!! Forcing program exit with SIGQUIT");
        ...
        GL1M("{}Exiting {}", cls->TAG, ProcessName());
...
private:
    ...
    function<void(void)> evt_init_callback = nullptr;
    uint8_t evt_task_started = 0;
...
public:
    ...
    void PushEvent(T &pkt_evt)
    {
        if constexpr (std::is_same_v<pkt_evt_t, T>) {
            if (likely((pkt_evt.pkt_buf != pkt_evt.o_pkt_buf) && pkt_evt.pkt_len)) {
     ...
     * @return void
     */
    void SetEventHandler(function<void(T &)> fcn, bool realtime = false, function<void()> usr_init_fcn = nullptr)
    {
        evt_callback = fcn;
        evt_init_callback = usr_init_fcn;

        if (evt_task_started)
            return;

        folly::USPSCQueue<int, true, 1> queue_init;

        thread *t = new thread([this, realtime, &queue_init] {
            if (realtime)
                enable_rt_scheduler();
            else
                enable_idle_scheduler();

            if (evt_init_callback)
                evt_init_callback();

            int ready = 1;
            queue_init.enqueue(1);

            T pkt_evt = {0};

            while (true) {
                queue_evt.dequeue(pkt_evt);
                if (unlikely(!evt_callback))
                    continue;
        ...
        int thread_started;
        queue_init.dequeue(thread_started);
...
private:
    ...
    {
        // Name thread
        string thread_name = fmt::format("ph_{}", string_split(proto_name, ":").back());
        ...
        if (!wd) {
            GL1R("{}wd_init for {} failed", TAG, proto_name);
     ...
     * If wd_init() fails, a message is logged using GL1R() and the function returns immediately.
    ...
    template <class T>
    void handler_simple_startup_with_driver(T &Driver,
                                            WD_HANDLER_PKT_SMPL_DRV_TYPE user_callback,
                                            bool enable_rt,
                                            string handler_name)
    {
        // Name thread
        string thread_name = handler_name;
        if (thread_name.size() >= 15) // Truncate name if needed
            thread_name = string(&thread_name.c_str()[0], &thread_name.c_str()[14]);
        pthread_setname_np(pthread_self(), thread_name.c_str());
        ...
        if (!run) {
            GL1R("{}Handler \"{}\" failed to initialize!", TAG, handler_name);
            return;
        }
        ...
        user_callback(PacketEventQueue, Driver);
    }
...
public:
                                    ...
                                    const char *proto_name = NULL)
    {
        ...
        LauncherList.push_back([this, proto_name_str, &DriverPtr, fcn, direction, dissection_mode, realtime]() -> bool {
            ...
            GL1Y("{}Added \"{}\", Dir:{}, Realtime:{}, TID:{}", TAG, proto_name_str, (int)direction, realtime, init_val);
            return init_val;
        });
    ...
    template <class T>
    bool AddSimplePacketHandlerWithDriver(T &DriverPtr,
                                          WD_HANDLER_PKT_SMPL_DRV_TYPE fcn,
                                          bool realtime = true,
                                          string handler_name = "pkt_handler")
    {
        LauncherList.push_back([this, &DriverPtr, fcn, realtime, handler_name]() -> bool {
            threads_list.push_back(thread(&WDPacketHandler::handler_simple_startup_with_driver<T>, this, ref(DriverPtr), fcn, realtime, handler_name));
            long init_val;
            // Wait thread to correctly initialize wdissector instance
            startup_queue.dequeue(init_val);
            GL1Y("{}Added \"{}\", Realtime:{}, TID:{}", TAG, handler_name, realtime, init_val);
            return init_val;
        });
     ...
     * @return true always.
     */
    bool SetPacketEventsHandler(function<void(V &)> fcn, bool realtime = false, function<void(void)> usr_init_callback = nullptr)
    {
        PacketEventQueue.SetEventHandler(fcn, realtime, usr_init_callback);
        return true;
    }
     ...
     * @return None.
     */
    inline void PushPacketEvent(V &pkt_evt)
    {
...
private:
    ...
    bool global_timeout_callback()
    {
        ...
        GL1Y("{}Timeout triggered. TimeoutCounter={}, TotalTimeoutCounter={}", TAG, TimeoutCounter, TotalTimeoutCounter);
        ...
        GL1Y("{}Timeout Finished with {} callbacks executed", TAG, n_callbacks);
    ...
    void StartInternalCallback()
    {
        ...
        GL1Y("{}Initialized with {} ms", TAG, _timeout_ms);
...
public:
    ...
    //         GL1R("{}Not enabled in config. file", TAG);
    //         return;
    //     }
    ...
    void CallIndicateTimeout(T &Instance, function<void(void)> pre_timeout_callback = nullptr)
    {
        ...
        if constexpr (!has_indicate_timeout) {
            GL1M("{}Class {} does not have IndicateTimeout(WDGlobalTimeout&, int) function", TAG, CLASS_NAME(T));
            return;
        }
        ...
        TimeoutHandlers.push_back([this, &Instance, has_indicate_timeout, pre_timeout_callback]() -> bool {
            ...
            GL1("{}Signalling {}", TAG, CLASS_NAME(T));
            if constexpr (has_indicate_timeout) {
    ...
    void CallRestart(T &Instance, function<void(void)> pre_timeout_callback = nullptr)
    {
        TimeoutHandlers.push_back([this, &Instance, pre_timeout_callback]() -> bool {
            // Only call IndicateTimeout() if instance implements it
            GL1("{}Restarting {}", TAG, CLASS_NAME(T));
            if (pre_timeout_callback)
                pre_timeout_callback();
    ...
    void SetTimeoutCallback(function<bool(WDGlobalTimeout &, void *)> fcn, void *data_ptr = NULL)
    {
        if (unlikely(!fcn))
            return;
    ...
    void SetTimeoutCallback(function<bool()> fcn)
    {
        if (unlikely(!fcn))
            return;
...
public:
    ...
    void IndicateCrash()
    {
        // IndicateCrash(fmt::format("[Crash] Crash detected at state \"{}\"", StateMachine.GetCurrentStateName()));
    }
