...
public:
    ...
    void writeLog(const string msg, bool error = false)
    {
        ...
        if (error)
            GL1("{}Packet Number:{}, Comment: {}", TAG, pkt_counter, msg);
    ...
    void write(lni::fast_vector<uint8_t> &pkt, const char *comment = NULL)
    {
        timeval time;
        gettimeofday(&time, NULL);
    ...
    void write(lni::fast_vector<uint8_t> &pkt, timeval &time, const char *comment = NULL)
    {
        write(&pkt[0], pkt.size(), time, comment);
    }
