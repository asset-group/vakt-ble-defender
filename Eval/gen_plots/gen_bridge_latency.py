...
data = pd.read_csv('latency_data_Anchored.txt', sep=" ", header=None)

def calc_stats(data):
    max_us = np.max(data)
    mean_us = np.mean(data)
    min_us = np.min(data)
    print('Samples: %d' % (data.count()))
...
sns.histplot(data=data[0], bins=150, element="step", 
    stat="frequency",
    label='Max: %d µs\nMin: %d µs\nMedian: %d µs' % (max_us1, min_us1, get_median_label(boxplot)),
...
# Graph Limits
axs.set_xlim(left=0, right=80)
ax_box.set_ylim(-0.7,0.7)

# Texts
axs.legend(labelspacing=0.8, prop={'style': 'italic'})
plt.xlabel('') 
#plt.xlabel('BLEDefender Bridging Latency (ms)') 
...
# Save/Show figure
#plt.savefig('interception_latency.pdf',bbox_inches='tight')  # requires latest lxml
