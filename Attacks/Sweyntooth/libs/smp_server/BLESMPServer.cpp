...
static PyObject *configure_connection(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "y*y*iii", &master_address, &slave_address, &slave_address_type, &blehost->io_capability, &blehost->auth_req))
    {
        return NULL;
    }
...
static PyObject *pairing_request(PyObject *self, PyObject *args)
{
    ...
    {
        PyObject *obj = Py_BuildValue("y#", packets_queue.front().buf, packets_queue.front().len);
        packets_queue.pop();
        return obj;
    }
    Py_RETURN_NONE;
...
{
    PyObject *obj = Py_BuildValue("y#", ltk, 16);
    return obj;
}
