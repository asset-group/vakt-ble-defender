...
class BLECentralMethods(object):  # type: HierarchicalGraphMachine
    ...
    conn_timeout = 100
    dongle_serial_port = '/dev/ttyACM3'
    ...
    def send_dh_check(self):
        ...
        if SM_Hdr in self.pkt:
            try:
                ...
                if hci_res:
                    pkt = BTLE(access_addr=self.conn_access_address) / BTLE_DATA() / L2CAP_Hdr() / \
                        SM_Hdr() / SM_DHKey_Check(dhkey_check='\x00'*16)
                    pkt.show()
                    self.send(pkt)
            except:
                pass
            ...
            if length == 6:  # 4 byte uuid, 2 2-byte handles
                ...
                for service in services:
                    try:
                        ...
                        uuid_128 = '-'.join(
                            (uuid_128[:8], uuid_128[8:12], uuid_128[12:16], uuid_128[16:20], uuid_128[20:]))
                        if end == -1:
                            end = 0xffff
                        if start == -1:
                            start = 0xffff
