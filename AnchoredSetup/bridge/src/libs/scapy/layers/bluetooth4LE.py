...
class NORDIC_BLE(Packet):
    ...
    def post_build(self, p, pay):
        if self.payload_len is None:
            #add lenght as little endian unsigned short 
            #p = p[:1] + chb(len(pay)+10) + p[2:]
            p = bytes([p[0]]) + struct.pack("<H", len(pay) + 10) + p[3:]
        return p + pay
...
class BTLE_DATA(Packet):
    ...
    fields_desc = [
        ...
        ByteField("Length", None), # BLE 4.2 and upwards can use 1 entire byte for length
    ]
...
class LL_REJECT_IND_EXT(Packet):
...
bind_layers(BTLE_DATA, BTLE_EMPTY_PDU, Length=0) # BTLE_DATA / CtrlPDU
...
bind_layers(CtrlPDU, LL_CONNECTION_PARAM_REQ, optcode=0x0F) # BLTE_DATA / CTRLPDU / LL_CONNECTION_PARAM_REQ
bind_layers(CtrlPDU, LL_CONNECTION_PARAM_RSP, optcode=0x10) # BLTE_DATA / CTRLPDU / LL_CONNECTION_PARAM_RSP
bind_layers(CtrlPDU, LL_REJECT_IND_EXT, optcode=0x11) # BLTE_DATA / CTRLPDU / LL_REJECT_IND_EXT
