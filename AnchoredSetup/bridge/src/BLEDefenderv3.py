...
# from libs.BLECrypto import *
# from libs.WDPacket_bkp import ValidatePacket
    ...
    while bridge_enabled:
        ...
        if data:
            ...
            if encryption_enabled:
                # After is encrypted
                print_l("TX Encryption")
                pkt = BTLE(data)
                pkt = encryptor.receive_encrypted(
            ...
            if BTLE_CONNECT_REQ in pkt:
                ...
                print_l(f"master_address={master_address}")
                print_l(f"master_address_raw={master_address_raw}")
                print_l(f"peripheral_address={slave_address}")
                print_l(f"slave_address_raw={slave_address_raw}")
                ...
                print_l(f"pairing_iocap={pairing_iocap}")
                print_l(f"paring_auth_request={paring_auth_request}")
                ...
                print_l("--- SMP Packet from central (real):")
                pkt.show()
    ...
    while connected_central:
        ...
        if data:
            ...
            # --------------- Process Link Layer Packets here ------------------------------------
                ...
                # conn_session_key = encryptor.bt_crypto_e(unhexlify("671f2eb5e22df543fdb8d01a78ab3f59")[
                #                                          ::-1], conn_skd[::-1])
                # Calculate the session key to initiate the encryption process
                conn_session_key = encryptor.bt_crypto_e(
            ...
            # Slave will send LL_ENC_RSP before the LL_START_ENC_RSP
                ...
                encryptor.config_encryption(
                    conn_session_key, conn_iv, conn_skd[::-1])
