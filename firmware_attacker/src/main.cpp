...
uint8_t radio_data_stop_pkt_forwarding = 0;   // Disable packet forwading to the host
  ...
  if (NRF_RADIO->EVENTS_END)
  {
    ...
    if (transmitting)
    {
      ...
      if (wait_end_of_conn_req) // After end of transmitted packet, schedule a connection window
      {
        ...
        radio_data_mode = 1;
        start_timer(change_data_channel, (connection.window_offset * 1250) + 1250);
...
void loop()
{
  ...
  while (Serial.available()) // If computer sent packet via USB Serial
  {
    ...
    case CMD_SET_STOP_PKT_FORWARDING:
      Serial.readBytes(&radio_data_stop_pkt_forwarding, 1);
      break;
